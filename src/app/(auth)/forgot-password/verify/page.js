...
import { forgotPasswordVerification,resendCode } from "@/api/apiHandler";
...
export default function PinVerification() {
  ...
  useEffect(() => {
    const lastResendTime = localStorage.getItem("lastResendTimeForgot");
    if (lastResendTime) {
      ...
      // First-time visit
      localStorage.setItem("lastResendTimeForgot", Date.now().toString());
      setTimer(60);
      setCanResend(false);
    }
  }, []);
  ...
  const handleVerify = async () => {
    ...
    let res=await forgotPasswordVerification(data);
    if (res.code == 1) {
        toast.success(res.message);
        let {resetToken}=res.data;
        localStorage.removeItem('user_id');
        router.replace(`/forgot-password/update-password?resetToken=${resetToken}`);
    } else {
